...
class Category:
    ...
    def __str__(self):
        ...
        tmp.extend(
            f'{entry["description"][:23].ljust(23)}{"{0:>7.2f}".format(entry["amount"])}' for entry in self.ledger)
...
def create_spend_chart(categories):
    ...
    spending = [abs(sum(entry["amount"] for entry in category.ledger
                        if entry["amount"] < 0)) for category in categories]
    total_spending = sum(spending)
    category_spending = [floor(x / total_spending * 10) * 10 for x in spending]
    output.extend(' '.join([f"{y}|".rjust(4), (' ' * 2).join([' ' if x < y else 'o' for x in category_spending]), ' '])
    ...
    output.extend(' ' * 5 + (' ' * 2).join([label[i] for label in labels]) + (' ' * 2) for i in range(longest_label))
    return '\n'.join(output)
